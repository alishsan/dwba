...
;; This is critical for ensuring accuracy

;; Load dependencies first
(try
  (require 'complex)
  (catch Exception e
    (println "Note: complex namespace not found, but may not be needed for Numerov functions")))

;; Load the functions namespace
;; In CIDER, you may need to load this manually first:
;; (load-file "src/functions.clj")
;; Or ensure the namespace is compiled

(require '[functions :refer :all] :reload)
  ...
  (doseq [row table]
    (let [h (:h row)
          ...
          bessel-paper (condp = h
                         ...
                         0.01 7.5e-10
                         0.0)
          ;; Check if within 20% (reasonable tolerance for rounding)
          naive-match (if (> naive-paper 0)
                       (< (Math/abs (- naive-code naive-paper)) (* 0.2 naive-paper))
                       false)
          bessel-match (if (> bessel-paper 0)
                        (< (Math/abs (- bessel-code bessel-paper)) (* 0.2 bessel-paper))
                        false)
...
;; ============================================
;; 2. Verify Wronskian Stability (if in paper)
;; ============================================
...
;; ============================================
;; 4. Detailed Check for h=0.01 (most precise)
;; ============================================
(println "4. DETAILED CHECK: h = 0.01 fm")
(println "================================")
(let [h 0.01
      u-bessel (solve-numerov (:e paper-params) (:l paper-params) 
                               (:v0 paper-params) (:rad paper-params) 
                               (:diff paper-params) h (:r-boundary paper-params))
      u-naive (solve-numerov-naive (:e paper-params) (:l paper-params) 
                                    (:v0 paper-params) (:rad paper-params) 
                                    (:diff paper-params) h (:r-boundary paper-params))
      delta-exact (exact-phase-shift-numerov (:e paper-params) (:l paper-params)
                                             (:v0 paper-params) (:rad paper-params) 
                                             (:diff paper-params) (:r-boundary paper-params))
      delta-bessel (phase-shift-from-numerov u-bessel h (:r-boundary paper-params) 
                                              (:e paper-params) (:l paper-params))
      delta-naive (phase-shift-from-numerov u-naive h (:r-boundary paper-params) 
                                            (:e paper-params) (:l paper-params))
